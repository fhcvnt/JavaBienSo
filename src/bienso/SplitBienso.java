...
public class SplitBienso {
	...
	public static void main(String[] args) {
		...
		try {
			SplitBienso window = new SplitBienso();
			window.open();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Mat getImageBiensoAuto2(Mat imagemat) {
		...
		try {
			Mat im = new Mat();
			im = imagemat.clone(); // ảnh gốc
			//Mat im = thayDoiDoSangTuongPhan(img, 4, 20);
			// resize ảnh nếu chiều ngang ảnh nhỏ hơn 230 (resize 1.5 lần)
			if (im.width() < 300) {
				// Mat resizeimage = new Mat();
				Imgproc.resize(im, im, new Size(im.width() * 1.5, im.height() * 1.5));
			...
			Mat equal_histogram = new Mat();
			//Imgproc.equalizeHist(noise_removal, equal_histogram);
			equal_histogram=noise_removal.clone();
	...
	// ======================================================================================================================
	public static Mat thayDoiDoSangTuongPhan(Mat src, double alpha, int beta) {
		Mat dst = src.clone();
		try {
			// Thay đổi độ sáng và độ tương phản của ảnh
			// ảnh f(x,y), g(x,y)=alpha*f(x,y)+beta (độ tương phản)
			// alpha=2
			// beta=30
			for (int i = 0; i < src.rows(); i++) {
				for (int j = 0; j < src.cols(); j++) {
					for (int k = 0; k < 3; k++) {
						...
						data[0] = alpha * data[0] + beta;
						data[1] = alpha * data[1] + beta;
