...
public class WindowBienSo {
	...
	public static void main(String[] args) {
		...
		try {
			WindowBienSo window = new WindowBienSo();
			window.open();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected void createContents() {
		...
		shellBienso.setText("Nhận dạng biển số - " + version);
				...
				if (filename != null) {
					...
					try {
						...
						if (image.getBounds().height * 800 / image.getBounds().width > 500) {
							lbImagedata.setBackground(
									resizeImage(image, image.getBounds().width * 500 / image.getBounds().height, 500));
						} else {
							lbImagedata.setBackground(
									resizeImage(image, 800, image.getBounds().height * 800 / image.getBounds().width));
						}
						lbImagedata.setText("");
					} catch (Exception ex) {
						ex.printStackTrace();
		...
		btnGetlicenseplate.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				try {
					...
					// cắt biển số
					SplitBienso catbienso = new SplitBienso();
					Mat bienso = catbienso.getImageBiensoAuto(im);
					if (bienso == null) {
						bienso = catbienso.getImageBiensoAuto(im);
					}
					if (bienso == null) {
						bienso = catbienso.getImageBienso100(im);
					}
					if (bienso == null) {
						bienso = catbienso.getImageBienso125(im);
					}
					if (bienso == null) {
						bienso = catbienso.getImageBienso150(im);
					}
					if (bienso == null) {
						bienso = catbienso.getImageBienso175(im);
					}
					if (bienso == null) {
						bienso = catbienso.getImageBienso200(im);
					}
					if (bienso == null) {
						bienso = catbienso.getImageBienso230(im);
					}
					if (bienso == null) {
						bienso = catbienso.getImageBienso250(im);
					}
					if (bienso == null) {
						bienso = catbienso.getImageBiensoAuto2(im);
					}

					// Tách ký tự
					//SplitCharFloodfill kytu = new SplitCharFloodfill();
					SplitChar kytu = new SplitChar();
					listcharimage.clear();
					...
					for (Mat imagechar : listcharimage) {
						try {
							...
							// HighGui.imshow("image "+sokytu, imagetest);
							// Ảnh từ hàm SplitCharFloodfill cho ra là ảnh nhị phân rồi, nên không cần
							// chuyển sang ảnh xám
